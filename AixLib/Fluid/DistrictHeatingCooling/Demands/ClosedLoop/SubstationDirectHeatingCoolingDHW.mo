...
model SubstationDirectHeatingCoolingDHW "Substation model for bidirctional low-temperature networks for buildings with 
...
equation
  ...
  connect(sourceHeating.ports[1], senTem3.port_a)
    annotation (Line(points={{136,-54},{130,-54},{130,-55}},
                                                          color={0,127,255}));
  ...
  connect(pumpHeating.port_b, del.ports[1])
    annotation (Line(points={{-44,0},{-2,0}}, color={0,127,255}));
  ...
  connect(del.ports[2], senTemHPin.port_a)
    annotation (Line(points={{2,0},{28,0},{28,1}},         color={0,127,255}));
...
end SubstationDirectHeatingCoolingDHW;
